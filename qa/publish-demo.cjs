...
(async()=>{
 const baseline=JSON.parse(fs.readFileSync('/tmp/vendvite-demo-baseline.json','utf8'));
 ...
 const response=await fetch('https://liasse.tech/admin/subscriber-pwas/433/import-from-testing',{
  ...
  body:JSON.stringify({source:'github',github:{owner:'doryza',repo:'liasse-tenant-exports',branch:'feat/vendvite-richard-demo'},note:'Add fictional Richard Tremblay RE/MAX demo page and homepage pitch in FR/EN; retire pricing assignment and always reveal 599 yearly. Preserve live generation 58 features.'}),signal:AbortSignal.timeout(180000)
 });
