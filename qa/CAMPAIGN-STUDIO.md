...
- Search a Québec address, neighbourhood or city, choose a radius (200 m–5 km), then click “Find addresses”. One circle shows the search area. Clicking or tapping the map places its centre; dragging moves it. Native zoom buttons, mouse wheel and touch pinch shrink or expand the radius. The map slider and search radius remain synchronized. No polygon drawing, map-picking mode, property-type filter, dwelling-count filter or source filter is presented.
...
- Map gestures preview the new circle without replacing the saved address selection. “Update addresses” applies it; “Keep my selection” restores the saved centre/radius. Checkout is disabled until the preview is applied or discarded. A newer gesture cancels an in-flight lookup, and late results cannot overwrite it.
- Expanded map view has a visible close action, Escape and keyboard focus containment. Programmatic address focus, popup auto-pan, fitting the area and resizing the map preserve the target circle.
...
- `node qa/map-selection-browser.cjs`: real zoom buttons, mouse wheel/drag, tap/pinch, slider keyboard control, apply/revert, exact saved exclusions, popup removal, fullscreen at 320×568, cancelled-search races and programmatic camera-movement isolation.
