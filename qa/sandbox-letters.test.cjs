...
const {create,root}=require('./harness.cjs'),mail=require(root+'/mailing-service-v7'),production=require(root+'/production-v2');
test('sandbox letters reproduce recipient output with stable private QR codes, never production access',async()=>{
 ...
 try{
  ...
  async function approveCampaign(c){const base=h.url+'/api/admin/campagnes/'+c.id,headers={...admin,'Content-Type':'application/json'};let r=await fetch(base+'/import-postal',{method:'POST',headers,body:JSON.stringify({revision:0,csv:production.template(c)})});assert.equal(r.status,200,await r.clone().text());const preparing=await h.db.get('SELECT * FROM broker_campaigns WHERE id=$1',[c.id]);r=await fetch(base+'/approve-postal',{method:'POST',headers,body:JSON.stringify({revision:preparing.production.revision,selected:preparing.production.candidates.map(a=>a.id),acknowledge:true})});assert.equal(r.status,200,await r.clone().text());}
  await approveCampaign(await h.db.get('SELECT * FROM broker_campaigns WHERE id=$1',[campaign.id]));
  ...
  const saved=await h.db.get('SELECT * FROM broker_campaigns WHERE id=$1',[campaign.id]);assert.equal(saved.is_test,1);assert.equal(saved.payment_status,'paid');assert.equal(saved.status,'processing');assert.equal(saved.paypal_mode,'sandbox');assert.equal(saved.mailed_at,null);assert.notEqual(mail.addresses(saved)[0].mailing_id,mail.addresses(saved)[1].mailing_id);
  assert.equal(targets.length,4);for(const [i,a] of mail.addresses(saved).entries())for(const lang of ['fr','en'])assert(targets.includes(h.url+'/courrier-test/'+saved.mailing_token+'/'+a.mailing_id+'?lang='+lang));
  ...
  for(const a of mail.addresses(saved)){
   ...
   assert.equal((await get('/courrier/'+saved.mailing_token+'/'+a.mailing_id,owner)).status,404);
  ...
  for(const [flag,mode] of [[0,'sandbox'],[1,'live']]){await h.db.run('UPDATE broker_campaigns SET is_test=$1,paypal_mode=$2 WHERE id=$3',[flag,mode,campaign.id]);assert.equal((await get(print,admin)).status,200);assert.equal(await mail.recipientForToken(h.db,saved.mailing_token,mail.addresses(saved)[0].mailing_id),null);}
  const liveCampaign=await h.db.get("INSERT INTO broker_campaigns(broker_id,kind,status,payment_status,is_test,paypal_mode,address_count,addresses) VALUES($1,'paid','confirmed','paid',0,'live',2,$2) RETURNING *",[broker.id,JSON.stringify(addresses)]);await approveCampaign(liveCampaign);const live=await get('/admin/campagnes/'+liveCampaign.id+'/lettres',admin);assert.equal(live.status,200);assert.doesNotMatch(await live.text(),/class="sandbox-mark"/);const liveSaved=await h.db.get('SELECT * FROM broker_campaigns WHERE id=$1',[liveCampaign.id]);assert.equal((await get('/courrier-test/'+liveSaved.mailing_token+'/'+mail.addresses(liveSaved)[0].mailing_id,admin)).status,302);assert.equal((await get('/courrier/'+liveSaved.mailing_token+'/'+mail.addresses(liveSaved)[0].mailing_id)).status,200);
  await h.db.run("UPDATE broker_campaigns SET is_test=1,paypal_mode='sandbox',status='cancelled' WHERE id=$1",[campaign.id]);assert.equal((await get(print,admin)).status,409);assert.equal((await get('/courrier-test/'+saved.mailing_token+'/'+mail.addresses(saved)[0].mailing_id,owner)).status,302);
