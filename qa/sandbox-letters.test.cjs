...
const {create,root}=require('./harness.cjs'),mail=require(root+'/mailing-service-v6'),production=require(root+'/production-v1');
test('sandbox letters reproduce recipient output with stable private QR codes, never production access',async()=>{
 ...
 try{
  ...
  const preparing=await h.db.get('SELECT * FROM broker_campaigns WHERE id=$1',[campaign.id]);preparing.production=production.stage(preparing,production.template(preparing));const approved=production.approve(preparing,preparing.production.candidates.map(a=>a.id));await h.db.run('UPDATE broker_campaigns SET production=$1::jsonb WHERE id=$2',[JSON.stringify(approved),campaign.id]);
  ...
  await h.db.run("UPDATE broker_campaigns SET is_test=0,paypal_mode='live' WHERE id=$1",[campaign.id]);assert.equal((await get('/courrier-test/'+saved.mailing_token+'/'+mail.addresses(saved)[0].mailing_id,admin)).status,302);const live=await get(print,admin);assert.doesNotMatch(await live.text(),/class="sandbox-mark"/);assert.equal((await get('/courrier/'+saved.mailing_token+'/'+mail.addresses(saved)[0].mailing_id)).status,200);
