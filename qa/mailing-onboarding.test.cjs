...
test('saved campaigns, personalized demos, free onboarding and campaign-only capture',async()=>{
 ...
 try{
  ...
  const demo=await req('/pwa/vendvite/invitation/'+agent.tag+'?lang=fr');assert.equal(demo.status,200);const html=await demo.text();assert.match(html,/4410 Pl\. de la Meuse/);assert.match(html,/Marie Tremblay/);assert.match(html,/window.VV_DEMO = true/);assert.match(html,/demarrer\/VV-/);assert.match(html,/Votre page de capture de prospect sans frais/);
