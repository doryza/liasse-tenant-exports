...
test('saved campaigns, personalized demos, free onboarding and campaign-only capture',async()=>{
 ...
 try{
  ...
  for(const path of ['/admin/sollicitations','/admin/sollicitations/'+c.id,'/admin/sollicitations/'+c.id+'/imprimer']){const r=await req(path,undefined,admin);assert.equal(r.status,200,path);const html=await r.text();assert.match(html,/Marie Tremblay/);if(path.endsWith('imprimer')){assert.equal((html.match(/class="sheet"/g)||[]).length,2);const windows=[...html.matchAll(/class="address">(.*?)<\/div>/gs)];assert.equal(windows.length,2);windows.forEach(w=>{assert.doesNotMatch(w[1],/Agence|514|https:/);assert.deepEqual([...w[1].matchAll(/<span>(.*?)<\/span>/g)].map(m=>m[1]),['MARIE TREMBLAY','1234 RUE DES ÉRABLES','LAVAL QC H7W 4Y4']);});}}
  const demo=await req('/pwa/vendvite/invitation/'+agent.tag+'?lang=fr');assert.equal(demo.status,200);const html=await demo.text();assert.match(html,/5025 Rue des Cageux/);assert.match(html,/Marie Tremblay/);assert.match(html,/window.VV_DEMO = true/);assert.match(html,/demarrer\/VV-/);assert.match(html,/Votre page de capture de prospect sans frais/);
