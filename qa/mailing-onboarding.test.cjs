...
test('saved campaigns, personalized demos, free onboarding and campaign-only capture',async()=>{
 ...
 try{
  ...
  const demo=await req('/pwa/vendvite/invitation/'+agent.tag+'?lang=fr');assert.equal(demo.status,200);const html=await demo.text();assert.match(html,/5025 Rue des Cageux/);assert.match(html,/Marie Tremblay/);assert.match(html,/window.VV_DEMO = true/);assert.match(html,/demarrer\/VV-/);assert.match(html,/Marie, cette page vous attend/);
