...
(async()=>{
 ...
 assert.equal(await page.locator('#csType,#csUnitFilter,#csSourceFilter,#csDraw,#csPick,#csSearchHere').count(),0);await page.locator('#csAddressSearch').fill('1000');assert.equal(await page.locator('.cs-address').count(),1);await page.locator('#csAddressSearch').fill('');
 ...
 assert.deepEqual(errors,[]);assert.equal(h.emails.length,0);console.log('Campaign browser checks passed: Québec overview, map right, selection/exclusion/undo, exact quote, cross-device drafts, conflict recovery, address search, letter dialog and FR/EN mobile widths. No external messages or payments.');
