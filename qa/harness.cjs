...
async function create(){
 ...
 const emails=[];
 const services={db,config:{businessName:'VendVite'},crypto:{randomBytes:n=>crypto.randomBytes(n).toString('hex'),sha256:s=>crypto.createHash('sha256').update(s).digest('hex')},admin:{isAdmin:req=>req.get('x-test-admin')==='yes'},email:{send:async m=>{emails.push(m);return {success:true}}}};
 ...
 for(const mount of ['/pwa/vendvite','']){
   const router=express.Router();router.use((req,res,next)=>{res.locals.base=mount+'/';req.tenantPath=p=>mount+p;req.tenantUrl=p=>'http://localhost:4798'+mount+p;next();});
 ...
 const server=await new Promise(resolve=>{const s=app.listen(4798,'127.0.0.1',()=>resolve(s))});
 return {pg,db,emails,services,server,close:async()=>{await new Promise(r=>server.close(r));await pg.close()}};
}
module.exports={create,root};
if(require.main===module)create().then(()=>console.log('Preview at http://127.0.0.1:4798/?vv_preview=visible'));
