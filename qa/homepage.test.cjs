...
test('one homepage version; retired conversion test cannot track or evaluate and old admin bookmarks stay protected',async()=>{
 ...
 for(const mount of ['','/pwa/vendvite']){
  ...
  const orders=await fetch(h.url+mount+'/admin/campagnes',{headers:{'x-test-admin':'yes'}});assert.equal(orders.status,200);const html=await orders.text();assert.doesNotMatch(html,/Test de conversion|admin\/conversions/);assert.match(html,/À traiter/);assert.match(html,/admin\/courtiers/);
 }
