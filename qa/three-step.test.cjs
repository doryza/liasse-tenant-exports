...
const {create,root}=require('./harness.cjs'),mail=require(root+'/mailing-service-v7');
test('recipient links stay stable, private and specific to confirmed mailings',async()=>{
 ...
 try{
  ...
  assert.equal((await fetch(h.url+prefix+'/'+'f'.repeat(32),{redirect:'manual'})).status,404);
  assert.equal((await fetch(h.url+prefix,{redirect:'manual'})).status,404,'campaign-only links show homeowner recovery');
  ...
  assert.equal((await fetch(h.url+prefix+'/'+one.addresses[0].mailing_id,{redirect:'manual'})).status,404);
  ...
  await h.db.run("UPDATE broker_campaigns SET status='cancelled' WHERE id=$1",[campaign.id]);assert.equal((await fetch(h.url+prefix,{redirect:'manual'})).status,404);
