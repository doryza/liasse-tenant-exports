...
test('own completed campaigns, lookback boundary, saved exclusions and checkout enforcement',async()=>{const h=await create();try{
 ...
 const raw=crypto.randomBytes(32).toString('hex');await h.db.run("INSERT INTO broker_sessions(broker_id,token_hash,idle_expires_at,absolute_expires_at) VALUES($1,$2,NOW()+INTERVAL '1 hour',NOW()+INTERVAL '1 hour')",[b.id,crypto.createHash('sha256').update(raw).digest('hex')]);const cookie='vv_broker_session='+raw,session=await(await fetch(h.url+'/api/espace/session',{headers:{cookie}})).json(),headers={cookie,'Content-Type':'application/json','x-vv-csrf':session.csrf};
