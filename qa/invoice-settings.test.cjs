...
test('invoice emails default to body-only with complete amounts and preserve sandbox notices',()=>{
 ...
 assert.equal('attachments' in msg,false);assert.equal(msg.skipInbox,true);assert.match(msg.text,/274,22/);assert.match(msg.text,/11,93/);assert.match(msg.text,/23,79/);assert.match(msg.text,/150/);assert.match(msg.text,/billing@example.test/);assert.match(msg.text,/Aucun paiement réel/);assert.doesNotMatch(msg.html,/<script>/);assert.doesNotMatch(msg.text,/jointe en PDF/);
...
test('tenant invoice settings persist, previews send nothing, and future sends use the saved configuration',async()=>{
 ...
 try{
  ...
  const one=await f.invoice('VV-2026-SETTINGS-1');assert.equal((await request('POST','/api/admin/ventes/factures/'+one.id+'/envoyer',{})).status,200);assert.equal(h.emails.length,1);assert.equal('attachments' in h.emails[0],false);
  ...
  const two=await f.invoice('VV-2026-SETTINGS-2');await request('POST','/api/admin/ventes/factures/'+two.id+'/envoyer',{});assert.equal(h.emails.length,2);const msg=h.emails[1];assert.equal(msg.attachments.length,1);assert.match(msg.text,/Camille Exemple, voici vos 150 lettres : 274,22/);assert.match(msg.text,/TPS-EXEMPLE/);assert.match(msg.text,/Agence de facturation/);assert.doesNotMatch(msg.html,/Télécharger ma facture/);
