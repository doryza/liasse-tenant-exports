...
async function fixture(h,mode='live',state='pending'){
...
test('campaign payments email a live invoice once and store a sandbox PDF preview without sending',async()=>{
 ...
 try{for(const mode of ['live','sandbox']){
  ...
  const i=await h.db.get('SELECT * FROM broker_invoices WHERE campaign_id=$1',[f.c.id]);assert(mode==='sandbox'?i.email_previewed_at&&!i.emailed_at:i.emailed_at&&!i.email_previewed_at);assert.equal(i.total_cents,27422);
  const emails=h.emails.filter(e=>e.to===f.b.email);assert.equal(emails.length,mode==='sandbox'?0:1);const email=mode==='sandbox'?(await h.db.get("SELECT payload FROM notification_jobs WHERE campaign_id=$1 AND kind='invoice_test'",[f.c.id])).payload:emails[0];assert.equal(email.attachments.length,1);assert.equal(email.attachments[0].filename,i.invoice_number+'.pdf');assert.match(email.text,/150/);assert.match(email.text,/274,22/);assert.doesNotMatch(email.text,/599|annuel|licence/);
  ...
  const before=calls.length;await fetch(f.url,{headers:{cookie:f.cookie},redirect:'manual'});assert.equal(calls.length,before,'Already paid returns do not recapture');assert.equal(h.emails.filter(e=>e.to===f.b.email).length,mode==='sandbox'?0:1);
...
test('existing invoices send to their recorded broker only; concurrent sends and unauthorized requests cannot duplicate them',async()=>{
 ...
 try{
  const f=await fixture(h,'live','paid');const i=await h.db.get("INSERT INTO broker_invoices(broker_id,kind,campaign_id,invoice_number,payment_key,payment_time,subtotal_cents,gst_cents,qst_cents,total_cents,is_test,paypal_mode) VALUES($1,'campagne',$2,'VV-TEST-2026-000034','old-camille-key',NOW(),23850,1193,2379,27422,0,'live') RETURNING *",[f.b.id,f.c.id]);
