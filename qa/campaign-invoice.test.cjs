...
test('confirmed campaign payments email an accurate PDF once in live and sandbox mode',async()=>{
 ...
 try{for(const mode of ['live','sandbox']){
  ...
  if(mode==='sandbox'){assert.match(text,/TOTAL SIMULÉ/);assert.match(email.text,/aucun courrier ne sera posté/);}else{assert.match(text,/TOTAL PAYÉ/);assert.match(email.text,/72 heures/);assert.doesNotMatch(email.subject,/TEST/);}
