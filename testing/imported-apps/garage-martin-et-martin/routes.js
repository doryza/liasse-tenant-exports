...
/** Vehicle sizes the garage prices tires and rust-proofing by, smallest first. */
const CLASSES = ['voiture', 'vus', 'pickup', 'gros'];

/** Bare paths 301 to their trailing-slash canonical form. */
const REDIRECTS = [
  ['/services', '/services/'], ['/rendez-vous', '/rendez-vous/'], ['/mon-compte', '/mon-compte/'],
  ['/voyants', '/voyants/'], ['/le-garage', '/le-garage/'], ['/contact', '/contact/'],
  ['/confidentialite', '/confidentialite/'],
  ['/en', '/en/'], ['/en/services', '/en/services/'], ['/en/appointment', '/en/appointment/'], ['/en/my-account', '/en/my-account/'],
  ['/en/warning-lights', '/en/warning-lights/'], ['/en/the-garage', '/en/the-garage/'], ['/en/contact', '/en/contact/'], ['/en/privacy', '/en/privacy/'],
  ['/reservation', '/rendez-vous/'], ['/booking', '/rendez-vous/'], ['/book', '/rendez-vous/'], ['/compte', '/mon-compte/'], ['/account', '/mon-compte/'],
  ['/a-propos', '/le-garage/'], ['/about', '/le-garage/'], ['/en/about', '/en/the-garage/'], ['/pneus', '/services/#gabarit'], ['/antirouille', '/services/#gabarit'],
];
...
module.exports = function (services) {
  ...
  function fail(req, res, e) {
    ...
    if (status >= 500) console.error('[garage-mm]', req.method, req.path, e && (e.stack || e.message));
  ...
  router.use(wrap(async (req, res, next) => {
    ...
    L.img = (url, t) => String(url || '').replace('/upload/', '/upload/' + t + '/');
    L.classesOf = (s) => String((s && s.vehicle_classes) || '').split(',').map((x) => x.trim()).filter(Boolean);
    L.CLASSES = CLASSES;
    L.clock = (hhmm) => {
      if (!hhmm) return '';
  ...
  // --- Public pages -----------------------------------------------------------
  page(['/', '/en/'], 'home', 'index', async () => ({ services: await publishedServices(), lights: LIGHTS.slice(0, 6) }));

  page(['/services/', '/en/services/'], 'services', 'services', async () => ({ services: await publishedServices() }));

  page(['/voyants/', '/en/warning-lights/'], 'lights', 'lights', async () => ({ lights: LIGHTS, services: await publishedServices() }));
  ...
  page(['/services/:slug/', '/en/services/:slug/'], 'service', 'service', async (req) => {
    ...
    if (!item) return null;
    // Same family first (the other tire or rust-proofing sizes), then the featured work.
    const fam = (s) => String(s.slug).split('-')[0];
    const related = all.filter((s) => s.slug !== item.slug && Number(s.bookable) && fam(s) === fam(item))
      .concat(all.filter((s) => s.slug !== item.slug && Number(s.bookable) && Number(s.featured) && fam(s) !== fam(item))).slice(0, 4);
    return { item, related, index: all.indexOf(item) + 1, lights: LIGHTS.filter((l) => l.service === item.slug) };
  });
  ...
  page(['/rendez-vous/', '/en/appointment/'], 'booking', 'booking', async (req) => {
    ...
    return {
      ...
      preselect,
      bookingConfig: { courtesyCars: cfg.courtesyCars, slotMinutes: cfg.slotMinutes, makes: cars.MAKES, towing: S.flag(req.site, 'towing_offered') },
    };
  });
  ...
  page(['/le-garage/', '/en/the-garage/'], 'about', 'about', async () => ({ services: await publishedServices() }));
  ...
  router.post('/api/appointments', services.auth.requireAuth, wrap(async (req, res) => {
    ...
    // Towing is only ever recorded when the garage offers it (setting towing_offered).
    const towing = b.towing && S.flag(raw, 'towing_offered') ? 1 : 0;
  ...
  require('./lib/adminRoutes')(router, {
    ...
    vehicleClasses: true, // services are sized: voiture · VUS · pick-up · gros
  });
