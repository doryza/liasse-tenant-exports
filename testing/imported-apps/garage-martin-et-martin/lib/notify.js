...
function card(raw, appt, kind, link) {
  ...
  const html = `<div style="margin:0;background:#e9e2d4;padding:28px 14px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
<div style="max-width:560px;margin:0 auto;background:#fffdf8;border-radius:6px;overflow:hidden;border:1px solid #13233f">
<div style="background:#13233f;border-bottom:3px solid #d8342b;padding:18px 26px;color:#f3ede1;font-weight:800;letter-spacing:.12em;text-transform:uppercase;font-size:14px">${esc(biz)}</div>
<div style="padding:28px 26px"><h1 style="margin:0 0 10px;font-size:24px;color:#13233f">${esc(title)}</h1><p style="margin:0 0 20px;color:#44484e;font-size:15px;line-height:1.55">${esc(lead)}</p>
<table style="width:100%;border-collapse:collapse;font-size:14px">${rows.filter((r) => r[1]).map((r) => `<tr><td style="padding:8px 0;color:#6b7078;width:34%;vertical-align:top;border-top:1px solid #e2dccf">${esc(r[0])}</td><td style="padding:8px 0;color:#13233f;font-weight:600;border-top:1px solid #e2dccf">${esc(r[1])}</td></tr>`).join('')}</table>
${link ? `<p style="margin:24px 0 0"><a href="${esc(link)}" style="display:inline-block;background:#13233f;color:#f3ede1;font-weight:700;text-decoration:none;padding:12px 18px;border-radius:4px">${esc(c.manage)}</a></p>` : ''}
  ...
  async function toGarage(raw, appt, link) {
    ...
    try {
      return await services.email.send({
        ...
        html: msg.html.replace('</h1>', `</h1><p style="white-space:pre-line;margin:0 0 16px;color:#13233f">${esc(extra)}</p>`).replace(esc(COPY.fr.requested[0]), 'Nouveau rendez-vous en ligne'),
      });
