...
function buildInvoicePdf(invoice, broker, issuer){
  ...
  text(50,713,9,'F1',isTest ? 'TEST PAYPAL · AUCUN PAIEMENT RÉEL' : 'REÇU DE PAIEMENT · ' + (isCampaign ? 'CAMPAGNE POSTALE' : 'ABONNEMENT ANNUEL'));
  ...
  text(326,622,12,'F2',bill?bill.legal_name:broker.full_name || '-');
  text(326,606,9,'F1',broker.agency || '');
  text(326,592,9,'F1',broker.email || '');
  if(bill){text(326,577,9,'F1',bill.line1.slice(0,42));text(326,563,9,'F1',[bill.city,bill.province,bill.postal_code].join(' ').slice(0,46));}
  ...
  fill('#111214');
  text(62,493,11,'F2',isCampaign ? 'Campagne postale VendVite' : 'Abonnement annuel VendVite');
  text(62,476,9,'F1',isCampaign ? (count ? count + ' lettres · ' : '') + 'Page de capture incluse à 0 $' : 'Page privée, capture de leads et espace courtier');
  text(432,493,11,'F1',money(invoice.subtotal_cents),118);
  stroke('#D5D8D3'); line(50,458,562,458,1);

  fill('#5F646C');
  text(62,436,9,'F1',isCampaign ? ('Secteur : ' + (campaign.centre_label || 'Campagne n° ' + invoice.campaign_id)).slice(0,100) : 'Période couverte : ' + dateFr(invoice.period_start) + ' au ' + dateFr(invoice.period_end));

  fill('#30343A');
  text(338,392,10,'F1','Sous-total',120); text(458,392,10,'F1',money(invoice.subtotal_cents),92);
  var taxRows=taxTools.rows(invoice),taxStep=taxRows.length>5?18:24;
  taxRows.forEach(function(l,index){
    var number=l.registration||(!invoice.tax_snapshot?(l.code==='QST'?issuer.qst:issuer.gst):'');
    text(258,368-index*taxStep,9,'F1',taxTools.label(l)+(number?' · '+number:''),200);
    text(458,368-index*taxStep,10,'F1',money(l.amount_cents),92);
  });
  var ruleY=Math.min(328,368-(taxRows.length-1)*taxStep-16),totalY=ruleY-25,boxY=totalY-79;
  stroke('#111214'); line(338,ruleY,562,ruleY,1);
  fill('#111214');
  text(320,totalY,13,'F2',isTest ? 'TOTAL SIMULÉ' : 'TOTAL PAYÉ',128); text(458,totalY,13,'F2',money(invoice.total_cents),92);

  fill('#F7F8F5'); c.push('50 '+boxY+' 512 50 re f');
  fill('#5F646C');
  text(62,boxY+26,9,'F2',isTest ? 'TRANSACTION PAYPAL SANDBOX' : 'PAYÉ AVEC PAYPAL');
  text(62,boxY+9,9,'F1',invoice.paypal_transaction_id || invoice.paypal_order_id || invoice.paypal_subscription_id || '');
  text(330,boxY+26,9,'F2',isCampaign ? 'CAMPAGNE' : 'ABONNEMENT');
  text(330,boxY+9,9,'F1',isCampaign ? 'N° ' + invoice.campaign_id : invoice.paypal_subscription_id || '');
