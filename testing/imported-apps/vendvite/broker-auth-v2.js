...
function safeNext(value){
  ...
  var local=u.pathname.slice(1),simple=['espace','espace/page','espace/courrier-cible','espace/pistes','espace/abonnement','espace/compte','espace/apercu','espace/commandes'].includes(local)||/^espace\/commandes\/[1-9]\d{0,9}$/.test(local)||/^espace\/campagnes\/[1-9]\d{0,9}\/(?:preparation|decision)$/.test(local);
  if(simple&&value.split('?')[0]===local){
    var allowed=['lang'];if(local==='espace/commandes')allowed.push('page');if(local==='espace/courrier-cible')allowed.push('campaignPage');
    ...
    if(keys.some(k=>k!=='lang'&&!/^[1-9]\d{0,4}$/.test(u.searchParams.get(k))))return 'espace';
