...
function register(router,services,h){const db=services.db;
 const protectAdmin=(req,res)=>{require('./broker-auth-v2').protect(res);if(!services.admin.isAdmin(req)){res.sendStatus(403);return false;}if(req.method!=='GET'&&(!sameOrigin(req)||!req.is('application/json'))){res.sendStatus(403);return false;}return true;};
 ...
 const invoices=await db.all("SELECT id,invoice_number,emailed_at,email_error FROM broker_invoices WHERE campaign_id=$1 AND broker_id=$2 ORDER BY id DESC",[campaign.id,campaign.broker_id]);
