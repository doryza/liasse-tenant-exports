...
const geography=require('./public/js/canada-geography-v1');
...
async function recipientForToken(db,campaignToken,recipientKey){
  ...
  return recipient&&!(campaign.production&&campaign.production.correctionPending)?{campaign,recipient}:null;
}
...
function addresses(c){if(c.production&&c.production.approvedAt&&Array.isArray(c.production.recipients))return c.production.recipients;return typeof c.addresses==='string'?JSON.parse(c.addresses):c.addresses||[];}
function addressLines(a){return [(a.unit?a.unit+'-':'')+a.numero+' '+a.rue,[a.ville,a.province||geography.province(a)||geography.postalProvince(a.postal)||'',a.postal||''].filter(Boolean).join(' ')];}
...
async function prepareRecipients(db,campaign){
  for(let attempt=0;attempt<3;attempt++){
    if(campaign.production&&campaign.production.approvedAt){if(campaign.mailing_token)return campaign;return await db.get('UPDATE broker_campaigns SET mailing_token=COALESCE(mailing_token,$1) WHERE id=$2 RETURNING *',[token(),campaign.id]);}
