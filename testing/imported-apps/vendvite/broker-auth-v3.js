...
function safeNext(value){
  if(typeof value!=='string'||value.length>300||/[\\\r\n#]/.test(value)||value.startsWith('/'))return 'espace';
  if(['espace','espace/page','espace/courrier-cible','espace/pistes','espace/abonnement','espace/compte','espace/apercu','espace/commandes'].includes(value))return value;
  if(/^espace\/commandes\/[1-9]\d{0,9}$/.test(value))return value;
  if(/^espace\/campagnes\/[1-9]\d{0,9}\/(?:preparation|decision)$/.test(value))return value;
  var u;try{u=new URL(value,'https://vendvite.invalid/');}catch(e){return 'espace';}
  if(u.origin!=='https://vendvite.invalid')return 'espace';
  var keys=Array.from(u.searchParams.keys());if(new Set(keys).size!==keys.length)return 'espace';
  var local=u.pathname.slice(1),simple=['espace','espace/page','espace/courrier-cible','espace/pistes','espace/abonnement','espace/compte','espace/apercu','espace/commandes','espace/tests','espace/tests/notifications'].includes(local)||/^espace\/commandes\/[1-9]\d{0,9}$/.test(local)||/^courrier-test\/[a-f0-9]{48}\/[a-f0-9]{32}$/.test(local)||/^espace\/campagnes\/[1-9]\d{0,9}\/(?:preparation|decision)$/.test(local);
  if(simple&&value.split('?')[0]===local){
    var allowed=['lang'];if(local==='espace/commandes'||local==='espace/tests/notifications')allowed.push('page');if(local==='espace/tests/notifications')allowed.push('campaign');if(local==='espace/pistes')allowed.push('mode');if(local==='espace/courrier-cible')allowed.push('campaignPage');
    if(keys.some(k=>!allowed.includes(k)))return 'espace';
    if(u.searchParams.has('lang')&&!['fr','en'].includes(u.searchParams.get('lang')))return 'espace';
    if(u.searchParams.has('mode')&&!['test','live'].includes(u.searchParams.get('mode')))return 'espace';
    if(keys.some(k=>!['lang','mode'].includes(k)&&!/^[1-9]\d{0,4}$/.test(u.searchParams.get(k))))return 'espace';
    return local+(keys.length?'?'+u.searchParams.toString():'');
  }
  if(u.pathname!=='/espace/campagne/retour'||keys.some(k=>!['token','mode','annule','PayerID'].includes(k)))return 'espace';
  // PayPal may append PayerID. Verify the order server-side; never need to
  // retain this payer identifier in the sign-in email or use it as authority.
  if(u.searchParams.has('PayerID')&&!/^[A-Za-z0-9]{1,64}$/.test(u.searchParams.get('PayerID')))return 'espace';
  var token=u.searchParams.get('token'),mode=u.searchParams.get('mode');
  if(!/^[A-Za-z0-9_-]{1,64}$/.test(token||'')||!['live','sandbox'].includes(mode||'live')||(u.searchParams.has('annule')&&u.searchParams.get('annule')!=='1'))return 'espace';
  return 'espace/campagne/retour?token='+encodeURIComponent(token)+'&mode='+(mode||'live')+(u.searchParams.has('annule')?'&annule=1':'');
}
...
function create(services){
  ...
  async function redeem(req,res,raw){
    ...
    var row=await db.get("WITH consumed AS (UPDATE broker_tokens t SET used_at=NOW() FROM brokers b WHERE t.broker_id=b.id AND t.token_hash=$1 AND t.purpose IN ('access','login') AND t.used_at IS NULL AND t.revoked_at IS NULL AND t.expires_at>NOW() AND b.status IN ('invited','active','cancelled','expired') RETURNING t.broker_id,t.invitation_id), verified_broker AS (UPDATE brokers b SET email_verified_at=COALESCE(b.email_verified_at,NOW()) FROM consumed c WHERE b.id=c.broker_id), verified_claim AS (UPDATE solicitation_claims s SET verified_at=COALESCE(s.verified_at,NOW()) FROM consumed c WHERE s.agent_id=c.invitation_id AND s.broker_id=c.broker_id RETURNING s.agent_id,s.broker_id), attached AS (UPDATE solicitation_agents a SET broker_id=v.broker_id,claimed_at=COALESCE(a.claimed_at,NOW()) FROM verified_claim v WHERE a.id=v.agent_id AND (a.broker_id IS NULL OR a.broker_id=v.broker_id)) INSERT INTO broker_sessions(broker_id,token_hash,device_label,idle_expires_at,absolute_expires_at) SELECT broker_id,$2,$3,NOW()+INTERVAL '30 days',NOW()+INTERVAL '90 days' FROM consumed RETURNING *",[hash(raw),hash(sessionRaw),device(req)]);
    if(!row)return null;
