...
async function matches(db,brokerId,addresses,filter,now=new Date()){
  ...
  const campaigns=await db.all("SELECT addresses,city,province,COALESCE(mailed_at,created_at) AS targeted_at FROM broker_campaigns WHERE broker_id=$1 AND is_test=0 AND status IN ('confirmed','mailed') AND (kind='included' OR payment_status='paid') AND ($2::timestamptz IS NULL OR COALESCE(mailed_at,created_at)>=$2::timestamptz) ORDER BY COALESCE(mailed_at,created_at) DESC",[brokerId,since]);
  const wanted=new Set(addresses.map(a=>key(a))),latest=new Map();
  for(const campaign of campaigns){for(const a of campaign.addresses||[]){const k=key(a,campaign.city,campaign.province);if(wanted.has(k)&&!latest.has(k))latest.set(k,new Date(campaign.targeted_at).toISOString());}}
