...
const DEMO_ADDRESS = '4410 Pl. de la Meuse, Laval, QC H7W 4Y4';
...
function parseAddresses(raw) {
  ...
  return blocks.map((block,index)=>{
    try {
      ...
      if(lines.slice(1).some(line=>measure.widthOfString(line)>3.05*72))throw Error('Une ligne d’adresse dépasse la fenêtre, même à 8 points. Abrégez-la avant de créer la campagne.');
...
function register(router,services,h) {
  ...
  router.get('/admin/sollicitations',h.requireAdmin,endpoint(async(req,res)=>{
    const campaigns=await db.all('SELECT c.*, (SELECT COUNT(*)::int FROM solicitation_agents a WHERE a.campaign_id=c.id) AS count, (SELECT COUNT(*)::int FROM solicitation_agents a WHERE a.campaign_id=c.id AND a.broker_id IS NOT NULL) AS registered FROM solicitation_campaigns c ORDER BY c.id DESC LIMIT 100');
    res.render('admin-sollicitations',Object.assign(await h.baseLocals(req),{active:'sollicitations',campaigns}));
  }));
  ...
  router.get('/admin/sollicitations/:id',h.requireAdmin,endpoint(async(req,res)=>{
    ...
    res.render('admin-sollicitation-detail',Object.assign(await h.baseLocals(req),{active:'sollicitations',campaign,agents}));
  }));
  router.get('/admin/sollicitations/:id/imprimer',h.requireAdmin,endpoint(async(req,res)=>{
    ...
    for(const agent of agents){for(const lang of campaign.format==='duplex'?['fr','en']:[campaign.format]){
      ...
      pages.push({agent,lang,url,qr:await services.qrcode.toDataURL(url,{errorCorrectionLevel:'M',margin:4,width:480})});
    }}
  ...
  router.get('/invitation/:tag',endpoint(async(req,res)=>{
    ...
    req.vvInvitation=agent;req.vvInitialAddress=DEMO_ADDRESS;
  ...
  router.post('/api/courtier/demarrer',endpoint(async(req,res)=>{
    ...
    const profile={agent_name:name,agency,agent_phone:phone,agent_email:email,agent_title:agent?agent.title:'',agent_photo_url:agent?agent.photo_url:'',links:[],mailing_terms_accepted_at:new Date().toISOString(),solicitation_tag:agent?agent.tag:null};
...
module.exports={parseAddresses,photoUrl,newTag,DEMO_ADDRESS,register};
