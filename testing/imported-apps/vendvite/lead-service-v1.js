...
function create(services){
 const db=services.db,outbox=require('./notification-outbox-v1').create(services);
 async function capture({broker,origin,input,lang='fr',workspaceUrl}){
  ...
  const existing=await db.get("SELECT id FROM broker_leads WHERE broker_id=$1 AND (submission_key=$2 OR (duplicate_key=$3 AND created_at>NOW()-INTERVAL '10 minutes')) ORDER BY id LIMIT 1",[broker.id,key,duplicate]);
  if(existing)return {success:true,duplicate:true};
  const count=await db.get("SELECT COUNT(*)::int AS n FROM broker_leads WHERE campaign_id=$1 AND recipient_id=$2 AND created_at>NOW()-INTERVAL '24 hours'",[origin.campaign.id,origin.recipient.mailing_id]);
  ...
  const messages=[{kind:'lead_agent',payload:brokerMessage(broker,l,workspaceUrl)}];if(l.email)messages.push({kind:'lead_receipt',payload:receipt(broker,l,lang)});
  // One statement commits both the request and its durable email jobs.
  const row=await db.get("WITH gate AS (INSERT INTO lead_request_limits(scope_hash) VALUES($15) ON CONFLICT(scope_hash) DO UPDATE SET requests=CASE WHEN lead_request_limits.window_started_at<=NOW()-INTERVAL '24 hours' THEN 1 ELSE lead_request_limits.requests+1 END,window_started_at=CASE WHEN lead_request_limits.window_started_at<=NOW()-INTERVAL '24 hours' THEN NOW() ELSE lead_request_limits.window_started_at END WHERE lead_request_limits.requests<5 OR lead_request_limits.window_started_at<=NOW()-INTERVAL '24 hours' RETURNING scope_hash), lead AS (INSERT INTO broker_leads(broker_id,name,email,phone,address,lat,lng,timeframe,status,campaign_id,recipient_id,lang,submission_key,duplicate_key,duplicate_window,response_due_at) SELECT $1,$2,$3,$4,$5,$6,$7,$8,'nouveau',$9,$10,$11,$12,$13,FLOOR(EXTRACT(EPOCH FROM NOW())/600)::bigint,NOW()+INTERVAL '24 hours' FROM gate ON CONFLICT DO NOTHING RETURNING *), jobs AS (INSERT INTO notification_jobs(job_key,kind,payload,broker_id,lead_id,campaign_id) SELECT 'lead:'||lead.id||':'||(j->>'kind'),j->>'kind',j->'payload',lead.broker_id,lead.id,lead.campaign_id FROM lead CROSS JOIN jsonb_array_elements($14::jsonb) j RETURNING id) SELECT (SELECT id FROM lead) AS id,(SELECT COUNT(*) FROM gate)::int AS admitted,(SELECT COUNT(*) FROM jobs) AS jobs",[broker.id,l.name,l.email,l.phone,l.address,l.lat,l.lng,l.timeframe,origin.campaign.id,origin.recipient.mailing_id,lang,key,duplicate,JSON.stringify(messages),hash(origin.campaign.id+'/'+origin.recipient.mailing_id)]);
  ...
  return {success:true,duplicate:!row.id};
 }
 ...
 async function enqueueOverdue({limit=100,workspaceUrl='https://vendvite.app/espace/pistes'}={}){
  ...
  for(const l of rows)await outbox.enqueue('lead:'+l.id+':reminder',brokerMessage(l.broker,l,workspaceUrl,true),{kind:'lead_reminder',brokerId:l.broker_id,leadId:l.id,campaignId:l.campaign_id});
  return rows.length;
 }
 return {capture,update,enqueueOverdue};
}
