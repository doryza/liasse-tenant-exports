...
function buildInvoicePdf(invoice, broker, issuer){
  ...
  taxTools.rows(invoice).forEach(function(l,index){
    var number=l.registration||(!invoice.tax_snapshot?(l.code==='QST'?issuer.qst:issuer.gst):'');
    text(258,368-index*24,9,'F1',taxTools.label(l)+(number?' · '+number:''),200);
    text(458,368-index*24,10,'F1',money(l.amount_cents),92);
  });
  ...
  fill('#5F646C');
  text(62,250,9,'F2',isTest ? 'TRANSACTION PAYPAL SANDBOX' : 'PAYÉ AVEC PAYPAL');
  text(62,233,9,'F1',invoice.paypal_transaction_id || invoice.paypal_order_id || invoice.paypal_subscription_id || '');
  text(330,250,9,'F2',isCampaign ? 'CAMPAGNE' : 'ABONNEMENT');
  text(330,233,9,'F1',isCampaign ? 'N° ' + invoice.campaign_id : invoice.paypal_subscription_id || '');
