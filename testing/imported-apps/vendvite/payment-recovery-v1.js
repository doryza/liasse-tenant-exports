...
function create(services,h){
 ...
 async function reconcile(id,options={}){
  ...
  try{
   ...
   if((Number(c.is_test)===1)!==(cfg.mode==='sandbox')||cfg.mode!==c.paypal_mode)throw failure('MODE_MISMATCH');
 ...
 async function run(options={}){const rows=await db.all("SELECT c.id FROM broker_campaigns c WHERE c.kind='paid' AND c.is_test=0 AND c.paypal_mode='live' AND (c.payment_recovery ? 'createKey' OR c.payment_recovery ? 'checkedAt') AND c.payment_check_after<=NOW() AND (c.payment_lease_until IS NULL OR c.payment_lease_until<NOW()) AND ((c.payment_status='pending') OR (c.payment_status='paid' AND (COALESCE((c.payment_recovery->>'effectsComplete')::boolean,false)=false OR NOT EXISTS(SELECT 1 FROM broker_invoices i WHERE i.campaign_id=c.id AND i.emailed_at IS NOT NULL))) OR (c.payment_status='cancelled' AND c.paypal_order_id IS NOT NULL AND c.updated_at>NOW()-INTERVAL '7 days')) ORDER BY c.payment_check_after,c.id LIMIT $1",[Math.max(1,Math.min(50,Number(options.limit)||10))]);const results=[];for(const row of rows)results.push(await reconcile(row.id,options));return results.map(r=>({id:r.campaign?r.campaign.id:r.id,state:r.state}));}
...
module.exports={create,matches,completedCapture,approveLink};
