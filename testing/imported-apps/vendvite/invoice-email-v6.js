var taxTools=require('./canadian-tax-v2');
'use strict';
...
function message(invoice,broker,issuer,url,settings){
  ...
  var bill=taxTools.object(invoice.tax_snapshot).billing_address;
  if(bill)rows.push(['Adresse de facturation / Billing address',[bill.legal_name,bill.line1,bill.city,bill.province,bill.postal_code].join(' · ')]);
  rows.push(['Sous-total',values.sous_total]);
  taxTools.rows(invoice).forEach(function(l){var number=l.registration||(!invoice.tax_snapshot?(l.code==='QST'?issuer.qst:issuer.gst):'');rows.push([taxTools.label(l)+(number?' · '+number:''),invoiceTools.money(l.amount_cents)]);});
  rows.push([test?'Total simulé':'Total payé',values.total]);
  ...
  // The editable body owns the fulfillment wording; sandbox notices always remain visible.
  var notice=test?'Document de test sans valeur comptable. Aucun paiement réel n’a été encaissé'+(isCampaign?' et aucun courrier ne sera posté pour ce test.':'.')+' / Test document with no accounting value. No real payment was collected'+(isCampaign?' and no letters will be mailed for this test.':'.'):'';
  ...
  var html='<div style="font-family:Arial,sans-serif;background:#f5f3ee;padding:28px;color:#242820"><div style="max-width:600px;margin:auto;background:#fff;padding:28px;border:1px solid #d8d5cb;border-radius:8px">'
    ...
    +'<p style="line-height:1.6">'+paragraphs(intro)+'</p><p><strong>Facture '+escape(invoice.invoice_number)+'</strong></p>'
    ...
    +'<p style="font-size:15px;line-height:1.6">'+paragraphs(footer)+'</p>'
    ...
    +'<p style="font-size:13px;color:#595a54;line-height:1.6;border-top:1px solid #ddd;padding-top:16px">Émise par / Issued by<br>'+issuerLines.map(escape).join('<br>')+'</p></div></div>';
  var result={to:broker.email,replyTo:{email:issuer.email},skipInbox:true,subject:(test?'[TEST PAYPAL] ':'')+configTools.expand(settings.invoice_email_subject,values).replace(/[\r\n]+/g,' ').slice(0,220),html:html,text:text};
...
async function send(options){
  ...
  if(Number(invoice.is_test)===1||invoice.paypal_mode==='sandbox'||campaign&&(Number(campaign.is_test)===1||campaign.paypal_mode==='sandbox')){
    await require('./notification-outbox-v2').create({db:db,email:options.email}).enqueue('invoice:'+invoice.id+':preview',message(invoice,broker,options.issuer,options.url,options.settings),{kind:'invoice_test',isTest:true,brokerId:broker.id,campaignId:invoice.campaign_id||null});
    const preview=await db.get('UPDATE broker_invoices SET email_previewed_at=COALESCE(email_previewed_at,NOW()),email_error=NULL WHERE id=$1 RETURNING email_previewed_at',[invoice.id]);
    return {status:'previewed',previewedAt:preview.email_previewed_at};
  }
