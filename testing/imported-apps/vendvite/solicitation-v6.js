...
function register(router,services,h) {
  ...
  router.get('/invitation/:tag',endpoint(async(req,res)=>{
    ...
    // Human-visible page activity is recorded by the page beacon, not raw GET requests.
    ...
    require('./broker-auth-v2').protect(res);
    res.set('Referrer-Policy','strict-origin-when-cross-origin');
    await h.renderBrokerPage(req,res,broker,false,true);
  }));
  async function bindClaim(agent,broker,raw){if(!agent)return;await db.run('INSERT INTO solicitation_claims(agent_id,broker_id) VALUES($1,$2) ON CONFLICT(agent_id,broker_id) DO NOTHING',[agent.id,broker.id]);await db.run('UPDATE broker_tokens SET invitation_id=$1 WHERE token_hash=$2 AND broker_id=$3',[agent.id,crypto.createHash('sha256').update(raw).digest('hex'),broker.id]);}
  ...
  router.post('/api/courtier/demarrer',endpoint(async(req,res)=>{
    ...
    if(agent&&agent.broker_id){const owner=await db.get('SELECT email FROM brokers WHERE id=$1',[agent.broker_id]);if(owner&&String(owner.email).toLowerCase()!==email)return res.status(409).json({error:fr?'Cette page est déjà réclamée. Utilisez Connexion courtier avec le compte associé.':'This page has already been claimed. Sign in with its associated account.'});}
    ...
    if(existing){
      ...
      if(require('./broker-auth-v2').allowed(existing))await h.brokerAuth.requestLink(req,email,async(broker,raw)=>{await bindClaim(agent,broker,raw);return h.sendMailingInvite(req,broker,raw);});
    ...
    await bindClaim(agent,broker,raw);
