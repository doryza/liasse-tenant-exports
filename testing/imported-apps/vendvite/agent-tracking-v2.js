...
function create(services){const db=services.db;
 async function issue(req,res,context){
  ...
  const token=crypto.randomBytes(24).toString('hex');await db.run('INSERT INTO agent_page_activity(token_hash,visitor_hash,agent_id,broker_id,kind,campaign_id,recipient_hash) VALUES($1,$2,$3,$4,$5,$6,$7)',[hash(token),hash(visitor),context.agentId||null,context.brokerId||null,context.kind,context.campaignId||null,context.recipientId?hash(context.recipientId):null]);return token;
 }
