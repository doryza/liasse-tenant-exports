(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory(require('./canada-geography-v1'));else root.VVCampaignModel=factory(root.VVCanadaGeography);})(this,function(G){
 'use strict';
 ...
 function canada(p){return G.canada(p);}
 ...
 function parse(elements,center,city,radius,polygon){
  ...
  var dedup=new Map();points.forEach(function(a){if(!a.rue||!valid(a)||!canada(a)||!inside(a,polygon))return;a.province=G.province(a);a.country='CA';a.metres=distance(center,a);if(a.metres>radius)return;var k=key(a),old=dedup.get(k);if(!old||old.source==='interpole'&&a.source==='point')dedup.set(k,a);});
 ...
 function sanitize(a){if(!a||!valid(a))return null;var out={numero:String(a.numero||'').trim().slice(0,20),rue:String(a.rue||'').trim().slice(0,160),ville:String(a.ville||'').trim().slice(0,120),postal:String(a.postal||'').trim().slice(0,10),unit:String(a.unit||'').trim().slice(0,20),lat:Math.round(Number(a.lat)*1e6)/1e6,lng:Math.round(Number(a.lng)*1e6)/1e6,source:a.source==='point'?'point':'interpole',metres:Number.isFinite(Number(a.metres))?Math.max(0,Math.round(Number(a.metres))):null};if(!out.numero||!out.rue||!canada(out)||(a.country&&String(a.country).toUpperCase()!=='CA'))return null;out.province=G.province(out);out.country='CA';out.id=key(out);return out;}
 return {norm:norm,street:street,key:key,valid:valid,canada:canada,province:G.province,regionBounds:G.regionBounds,distance:distance,integer:integer,classify:classify,inside:inside,parse:parse,summary:summary,filter:filter,sanitize:sanitize};
});
