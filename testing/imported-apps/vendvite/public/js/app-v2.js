    ...
    if(form){
      form.addEventListener('submit', function(e){
        ...
        fetch(leadUrl, { method:'POST', headers:{ 'Content-Type':'application/json' }, body:JSON.stringify(payload) })
          .then(function(r){return r.json().then(function(data){if(!r.ok)throw new Error(data.error==='server'?(T.errGeneric||'Please try again.'):data.error||T.errGeneric);return data;});})
