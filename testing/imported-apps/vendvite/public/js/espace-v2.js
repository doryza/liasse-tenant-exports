(function(){
  ...
  async function flushLead(id){
    ...
    while(record.pending){
      ...
      try{
        ...
        var lead=result.lead,en=document.documentElement.lang==='en',test=article.dataset.test==='1';
        if(lead){
          ...
          if(lead.analysis_delivered_at){progress.textContent=(test?(en?'Simulated delivery: ':'Remise simulée : '):(en?'Analysis delivered: ':'Analyse remise : '))+new Date(lead.analysis_delivered_at).toLocaleString(en?'en-CA':'fr-CA');article.querySelector('[data-lead-delivery-panel]')?.setAttribute('hidden','');var option=article.querySelector('.esp-lead-status option[value="évalué"]');option.disabled=false;article.querySelector('.esp-lead-status').value='évalué';}
          else if(lead.contacted_at)progress.textContent=(test?(en?'Simulated contact: ':'Contact simulé : '):(en?'Contact recorded: ':'Contact consigné : '))+new Date(lead.contacted_at).toLocaleString(en?'en-CA':'fr-CA');
          else if(lead.status==='fermé')progress.textContent=en?'Closed':'Fermée';
          if(lead.contacted_at||lead.status==='fermé'){progress.style.color='inherit';var overdue=article.querySelector('[data-test-overdue]');if(overdue)overdue.hidden=true;}
        }
  ...
  document.querySelectorAll('[data-lead-deliver]').forEach(function(btn){on(btn,'click',function(){
    ...
    if(!article.querySelector('[data-delivery-ack]').checked){flag.textContent=article.dataset.test==='1'?(en?'Confirm the simulated delivery first.':'Confirmez d’abord la remise simulée.'):(en?'Confirm that you have delivered the analysis first.':'Confirmez avoir d’abord remis l’analyse.');return;}
  ...
  document.querySelectorAll('[data-test-overdue]').forEach(function(btn){on(btn,'click',async function(){
    var article=btn.closest('.esp-lead'),flag=article.querySelector('[data-lead-save]'),en=document.documentElement.lang==='en';btn.disabled=true;flag.textContent=en?'Preparing simulated reminder…':'Préparation du rappel simulé…';
    try{var r=await apiFetch('api/espace/tests/leads/'+btn.dataset.testOverdue+'/overdue',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});var data=await r.json();if(!r.ok)throw Error(data.error||T('ws_retry'));article.querySelector('[data-lead-progress]').textContent=en?'TEST — first-contact reminder simulated':'TEST — rappel de premier contact simulé';flag.textContent=en?'Reminder preview is available in simulated emails. No email was sent.':'Le rappel est disponible dans les courriels simulés. Aucun courriel envoyé.';btn.hidden=true;}catch(e){flag.textContent=e.message||T('ws_retry');}finally{btn.disabled=false;}
  });});
