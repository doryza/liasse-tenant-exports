(function(){
  ...
  function initBanners(){
    if(window.VV_DEMO||window.VV_SANDBOX) return;
  ...
  function initForm(){
    ...
    async function submitLead(url,payload,retried){
      var response=await fetch(url,{method:'POST',headers:Object.assign({'Content-Type':'application/json'},window.VV_SANDBOX_CSRF?{'X-VV-CSRF':window.VV_SANDBOX_CSRF}:{}),body:JSON.stringify(payload)});
      var data=await response.json();
      if(window.VV_SANDBOX&&response.status===403&&data.code==='CSRF_INVALID'&&!retried){var refreshed=await fetch('api/espace/session');if(refreshed.ok){window.VV_SANDBOX_CSRF=(await refreshed.json()).csrf;return submitLead(url,payload,true);}}
      if(window.VV_SANDBOX&&response.status===401){if(q('testFormReconnect'))q('testFormReconnect').hidden=false;throw Error(document.documentElement.lang==='en'?'Your test session expired. Sign in again, then retry this form. Your entries are kept.':'Votre session de test a expiré. Rouvrez-la, puis réessayez ce formulaire. Vos renseignements sont conservés.');}
      if(!response.ok)throw Error(data.error==='server'?(T.errGeneric||'Please try again.'):data.error||T.errGeneric);
      return data;
    }
    var requestKey=null,requestFingerprint=null;
    function key(){return window.crypto&&crypto.randomUUID?crypto.randomUUID():Date.now().toString(36)+'_'+Math.random().toString(36).slice(2)+'_'+Math.random().toString(36).slice(2);}
    if(form){
      form.addEventListener('submit', function(e){
        ...
        var email=(q('leadEmail')?q('leadEmail').value:'').trim(),phone=(q('leadPhone')?q('leadPhone').value:'').trim(),digits=phone.replace(/\D/g,'');
        if(!email&&!phone){if(errEl)errEl.textContent=en?'Enter an email or phone number so your broker can reach you.':'Indiquez un courriel ou un numéro de téléphone pour que votre courtier puisse vous joindre.';if(q('leadEmail'))q('leadEmail').focus();return;}
        if(email&&(email.length>190||! /^[^\s@<>]+@[^\s@<>]+\.[^\s@<>]+$/.test(email))){if(errEl)errEl.textContent=en?'Check your email address.':'Vérifiez votre adresse courriel.';q('leadEmail').focus();return;}
        if(phone&&(phone.length>40||! /^[+\d\s().-]+$/.test(phone)||digits.length<10||digits.length>15||/^(\d)\1+$/.test(digits))){if(errEl)errEl.textContent=en?'Enter a valid phone number including its area code.':'Indiquez un numéro valide, avec l’indicatif régional.';q('leadPhone').focus();return;}
        var fingerprint=[name,address,email,phone].join('|');if(fingerprint!==requestFingerprint){requestKey=key();requestFingerprint=fingerprint;}
        var btn=q('sealBtn'); var orig=btn ? btn.textContent : '';
        if(btn){ btn.disabled=true; btn.textContent=T.submitting || '…'; }
        if(window.VV_PAGE_PREVIEW&&!window.VV_SANDBOX){if(errEl)errEl.textContent=document.documentElement.lang==='en'?'Preview only. No request has been sent.':'Aperçu seulement. Aucune demande envoyée.';if(btn){btn.disabled=false;btn.textContent=orig;}return;}
        if(window.VV_DEMO){ form.reset(); stampFiche(); return; }
        var payload={ submissionKey:requestKey,website:q('leadWebsite')?q('leadWebsite').value:'',mailingToken:window.VV_MAILING_TOKEN||null, mailingRecipient:window.VV_MAILING_RECIPIENT||null, name:name, email:((q('leadEmail') && q('leadEmail').value) || '').trim(), phone:((q('leadPhone') && q('leadPhone').value) || '').trim(), timeframe:((q('leadTime') && q('leadTime').value) || '').trim(), address:address, lat:((q('leadLat') && q('leadLat').value) || ''), lng:((q('leadLng') && q('leadLng').value) || '') };
        ...
        submitLead(leadUrl,payload)
          .then(function(){ stampFiche(); })
          .catch(function(error){ if(errEl) errEl.textContent=error.message||T.errGeneric||'Erreur.'; if(btn){ btn.disabled=false; btn.textContent=orig; } });
      });
    }
  }
