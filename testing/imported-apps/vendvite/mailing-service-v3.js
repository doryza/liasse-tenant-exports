...
async function campaignForToken(db,raw){
  ...
  return db.get("SELECT c.* FROM broker_campaigns c WHERE c.mailing_token=$1 AND (c.payment_status='paid' OR (c.kind='included' AND c.payment_status='none')) AND c.is_test=0 AND c.status IN ('confirmed','processing','mailed')",[raw]);
}
...
module.exports={isMailing,access,token,campaignForToken,recipientForToken,addresses,addressLines,deadline,prepareRecipients};
