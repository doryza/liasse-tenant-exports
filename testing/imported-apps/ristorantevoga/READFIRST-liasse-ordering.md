...
- **Name:** Restaurant Vogà
- **Liasse business id:** 649
- **Base URL:** https://restaurant-vog-saint860.liasse.tech
...
```js
const base = "https://restaurant-vog-saint860.liasse.tech";
...
## Ordering accounts & order history (this app — Option A, per-app mirror)

This app bridges Liasse's OTP-verified phone into a **real tenant user** and
**mirrors orders** into the tenant DB, so the current order + a 90-day history
follow the customer no matter where they navigate. **Liasse stays the source of
truth**; the tenant copy is a read-through cache.

**Why this works:** the Liasse OTP token is `jwt.sign({ phone, verified:true },
JWT_SECRET, '30m')`, and the *same* `JWT_SECRET` is handed to tenant route code
as `services.jwtSecret`. Every tenant DB also gets a platform-provisioned,
per-app `users` table (`phone TEXT UNIQUE`, `phone_verified`) plus
`services.auth` (`getUserByPhone`, `optionalAuth`/`requireAuth`, sessions). So
the bridge needs no new secret and no cross-origin hop.

Tenant routes added in `routes.js` (all same-origin to this app):

| Method & path | Purpose |
|---|---|
| `POST /api/order/session` | `{ token }` (the Liasse OTP token the client already holds) → `jwt.verify` with `services.jwtSecret`, assert `verified`, upsert `users` by phone, set a 30-day `tenant_token` session `{ userId, tenantSlug }` (cookie + Bearer). |
| `POST /api/order/record` | `optionalAuth`; `{ liasseOrderId }` → the **server** fetches `GET /order/orders/:id` from the restaurant host (authoritative totals/status — never trusts the client) and upserts a row in the tenant `orders` table under `user_id`. |
| `GET /api/order/history` | `optionalAuth`; the user's last-90-days `orders`, refreshing any non-terminal status from Liasse on read (no webhook needed). |

Client (`commander.ejs`): on OTP verify it calls `/api/order/session`; on place
it `saveActive()`s the order to `localStorage` and `/api/order/record`s it; the
status poller `clearActive()`s on a terminal state. A global **order pill**
(`partials/order-status-pill.ejs`, in the footer on every page) and a **resume
banner** on `/commander` keep the live order one tap away; **My orders** opens
the 90-day history.

Tenant `orders` table lives in `schema.sql` (survives Push-to-Live):
`user_id, liasse_order_id UNIQUE, order_number, order_type, status, items JSONB,
subtotal_cents, total_cents, tax_breakdown JSONB, customer_name, pickup_time,
created_at` (no FK to the platform-managed `users` table).

**Caveats / invariants:**
- **ASAP-at-counter orders carry no verified phone** (Liasse only verifies for
  scheduled/remote), so they get no tenant user and won't appear in server-side
  history — the `localStorage` current-order still tracks them.
- The `orders` table is a custom tenant table: a **version-restore**
  (`importTenantData`) recreates it from the snapshot, and the public
  `_db-snapshot.json` export could capture customer data — **exclude `orders`
  (and never export `users`) from the public sample** to avoid PII leaks.
- The OTP token has **no business binding** (`{phone,verified}` only); any valid
  Liasse OTP would mint a phone-keyed user here. Acceptable (short-lived,
  phone-only); harden by binding to the placed order if needed.
- The bridge **must not** call `services.auth` SMS methods (yearly-gated) — it
  verifies the Liasse-issued token directly.

**To generalize across AI builds (recommended, not done here):** promote the
bridge to platform infra — a built-in `services.auth.sessionFromLiasseToken()`
helper (removes the hand-rolled JWT re-sign) and a phone/token-authenticated
`POST /order/my-orders` endpoint on Liasse (single source of truth, no per-app
mirror). The AI builder can then emit the client snippet automatically.
