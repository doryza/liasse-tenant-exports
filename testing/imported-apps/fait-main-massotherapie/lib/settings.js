...
function urls(lang) {
  return lang === 'en' ? {
    home: 'en/', massotherapie: 'en/massage-therapy/', esthetique: 'en/skin-care/', ongles: 'en/nail-care/',
    corps: 'en/body-treatments/', forfaits: 'en/packages/', boutique: 'en/shop/', product: 'en/shop/product/',
    cart: 'en/cart/', order: 'en/order/', orders: 'en/my-orders/', location: 'en/room-rental/',
    about: 'en/about/', blog: 'en/journal/', article: 'en/journal/', contact: 'en/contact/',
    reservation: 'en/booking/', privacy: 'en/privacy/',
  } : {
    home: '.', massotherapie: 'massotherapie/', esthetique: 'esthetique/', ongles: 'ongles/',
    corps: 'soins-corps/', forfaits: 'forfaits/', boutique: 'boutique/', product: 'boutique/produit/',
    cart: 'panier/', order: 'commande/', orders: 'mes-commandes/', location: 'location-de-salle/',
    about: 'a-propos/', blog: 'blog/', article: 'blog/', contact: 'contact/',
    reservation: 'reservation/', privacy: 'confidentialite/',
  };
}

/** Treatment families, in the order the business presents them. */
...
module.exports = function (services) {
  ...
  async function init() {
    if (!initialized) initialized = (async () => {
      ...
      const defaults = {
        business_name: c.businessName || c.displayName || 'Fait Main Massothérapie',
...
Object.assign(module.exports, { localized, both, translate, flag, num, safeJSON, error, money, today, date, dateTime, slugify, taxes, urls, FAMILIES });
