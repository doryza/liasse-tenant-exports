(function () {
  ...
  function applyAuthVisibility(loggedIn) {
    ...
    document.dispatchEvent(new CustomEvent('fm:auth', { detail: { loggedIn: !!loggedIn } }));
  }
  ...
  if (messageForm) {
    messageForm.addEventListener('submit', async function (event) {
      ...
      try {
        ...
        messageForm.reset();
        if (status) { status.textContent = result.message || data.t.sent; status.hidden = false; status.style.color = 'var(--sage-deep)'; }
        toast(result.message || data.t.sent);
      } catch (e) {
        if (status) { status.textContent = e.message; status.hidden = false; status.style.color = 'var(--rose-deep)'; }
