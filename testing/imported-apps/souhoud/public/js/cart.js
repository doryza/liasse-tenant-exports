(function () {
  ...
  function write(lines) {
    ...
    paintCount();
    document.dispatchEvent(new CustomEvent('sh:cart'));
  }
  ...
  // --- School list: add every one-click item at once -----------------------
  var bundleButton = document.querySelector('[data-add-bundle]');
  var bundleNode = document.getElementById('bundle-data');
  if (bundleButton && bundleNode) {
    bundleButton.addEventListener('click', function () {
      var items = [];
      try { items = JSON.parse(bundleNode.textContent); } catch (e) { items = []; }
      items.forEach(function (item) {
        add({ slug: item.slug, name: item.name, price: item.price, image: item.image, quantity: item.quantity });
      });
      bundleButton.disabled = true;
      if (window.App) App.toast(data.t.list_added);
      setTimeout(function () { window.location.assign(data.urls.cart); }, 700);
    });
  }
  ...
  document.addEventListener('sh:cart', renderCart);
