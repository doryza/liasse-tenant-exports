(function () {
  ...
  function requestKey(lines, fulfilment) {
    ...
    try {
      var stored = window.sessionStorage.getItem('sh_req_' + hash);
      if (stored) return stored;
      var fresh = String(hash) + '-' + Date.now().toString(36);
      window.sessionStorage.setItem('sh_req_' + hash, fresh);
      return fresh;
  ...
  function wireOrder() {
    ...
    function render() {
      ...
      var trackEl = root.querySelector('[data-order-tracking]');
      if (trackEl) { trackEl.textContent = order.tracking ? t.status_shipped + ' · ' + order.tracking : ''; trackEl.hidden = !order.tracking; }
    ...
    document.addEventListener('sh:auth', function (event) { if (event.detail.loggedIn) load(); });
  }
