...
async function price(services, raw, cart, fulfilment) {
  ...
  for (const want of wanted) {
    ...
    // A price the owner has not confirmed is never charged, whatever the row says.
    if (!product.price_verified) throw S.error('price_unconfirmed_error', 409);
...
async function reference(db, id) {
  return 'SH-' + String(10000 + Number(id)).slice(-5);
}
  ...
  try {
    if (S.flag(raw, 'live_actions_enabled') && services.config.contactEmail) {
      ...
      await services.email.send({
        ...
        replyTo: email,
        subject: `Nouvelle commande Souhoud — ${name}`,
        text: `Une commande a été enregistrée sur le site.\n\nClient : ${name}\nCourriel : ${email}\nTéléphone : ${phone || '—'}\nMode : ${fulfilment === 'pickup' ? 'Cueillette en boutique' : 'Livraison — ' + [address.line, address.city, address.province, address.postal].filter(Boolean).join(', ')}\n\n${lines}\n\nTotal : ${(quote.totalCents / 100).toFixed(2)} $ CAD\nNote : ${note || '—'}`,
      });
