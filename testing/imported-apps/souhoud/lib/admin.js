...
function clean(module, body, old = {}) {
  ...
  if (module.key === 'products' && (out.price_cents == null || out.price_cents < 0)) throw S.error('invalid');
  if (module.key === 'products' && out.price_verified && !(out.price_cents > 0)) throw S.error('invalid');
  if (module.key === 'bundles') {
    // Keep only well-formed "slug x qty" lines so the public page never breaks.
    const items = S.bundleItems(out.items);
    out.items = JSON.stringify(items.map((i) => i.slug + ' x ' + i.quantity));
  }
  return out;
}
...
async function stats(services, modules) {
  const db = services.db;
  const counts = {};
  for (const m of modules) counts[m.key] = Number((await db.get('SELECT COUNT(*) AS n FROM ' + m.key)).n);
  let userCount = 0; let pushSubscriberCount = 0;
  ...
  const sales = await db.get("SELECT COUNT(*) FILTER(WHERE payment_status='paid') AS paid_orders,COALESCE(SUM(total_cents) FILTER(WHERE payment_status='paid'),0) AS revenue_cents,COUNT(*) FILTER(WHERE status='awaiting_payment') AS awaiting FROM orders");
  const unread = await db.get("SELECT COUNT(*) AS n FROM messages WHERE status='new'");
  const prices = await db.get('SELECT COUNT(*) AS n FROM products WHERE published=1 AND price_verified=0');
  return {
    counts, userCount, pushSubscriberCount,
    totalVisits: Number(v.total), recentVisits: Number(v.recent),
    paidOrders: Number(sales.paid_orders), revenueCents: Number(sales.revenue_cents),
    awaitingOrders: Number(sales.awaiting), newMessages: Number(unread.n), unconfirmedPrices: Number(prices.n),
  };
}
